...
import {  purchaseColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function PurchasesData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
 ...
 const newData = useMemo(()=>{
  ...
  return (
     <div>
      <FixedHeader
             linkUrl="/dashboard/purchases/new"
             textValue="Purchases"
             numberData={newData?.length | 0}
             isHidden={!permission?.purchasesDataCreate}
           />
          ...
          <DataTable
                    ...
                    onDeleted={permission?.purchasesDataDelete}
                    onViewInvoice={permission?.purchasesDataViewInvoice}
                    linkViewInvoice="/dashboard/purchases/invoice/"
                    linkToCreate="/dashboard/purchases/new"
                    titleToCreate="purchases"
                    endPoint="purchases"
