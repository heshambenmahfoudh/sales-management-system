...
import { categoryColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function CategoriesData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
...
const newData = useMemo(()=>{
  ...
  return (
     <div>
        <FixedHeader
        linkUrl="/dashboard/inventory/categories/new"
        textValue="Categories"
        numberData={newData?.length | 0}
        isHidden={!permission?.categoriesDataCreate}
      />
          ...
          onDeleted={permission?.categoriesDataDelete}
          onUpdated={permission?.categoriesDataUpdate}
          linkToUpdate="/dashboard/inventory/categories/update/"
          linkToCreate="/dashboard/inventory/categories/new"
          titleToCreate="categories"
          endPoint="categories"
