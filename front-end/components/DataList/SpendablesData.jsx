...
import { spendableColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function SpendablesData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
 ...
 const newData = useMemo(()=>{
  ...
  return (
     <div>
     <FixedHeader
             linkUrl="/dashboard/spendables/new"
             textValue="Spendables"
             numberData={newData?.length | 0}
             isHidden={!permission?.spendablesDataCreate}
           />
            ...
            <DataTable
                     data={newData}
                     columns={spendableColumn}
                     onView={permission?.spendablesDataView}
                     onDeleted={permission?.spendablesDataDelete}
                     linkToView="/dashboard/spendables/view/"
                     linkToCreate="/dashboard/spendables/new"
                     titleToCreate="spendables"
                     endPoint="spendables"
                     resourceName="Spendable"
                   />
        </div>
  )
}
