...
import { customerDelayColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function CustomersDelayData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
 ...
 const newData = useMemo(()=>{
  ...
  return (
    ...
    <div className=' overflow-auto md:mt-6 mt-4  md:mx-5 mx-3 '>
        ...
        <DataTable
                     data={newData}
                     columns={customerDelayColumn}
                     onUpdated={permission?.customersDelayedUpdate}
                     onDeleted={permission?.customersDelayedDelete}
                     linkToUpdate="/dashboard/customers/customers-delayed/update/"
                     endPoint="customers/customers-mony"
                     resourceName="Customer Delay"
                   />
