...
import {    productTransferColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function ProductsTransferData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
 ...
 const newData = useMemo(()=>{
  ...
  return (
     <div>
    <FixedHeader
        linkUrl="/dashboard/inventory/products-transfers/new"
        textValue="Products Transfers"
        numberData={newData?.length | 0}
        isHidden={!permission?.productsTransfersDataCreate}
      />
        <div className=' overflow-auto md:mt-6 mt-4  md:mx-5 mx-3 '>
          ...
          onView={
            permission?.productsTransfersDataView
          }
          onDeleted={
            permission?.productsTransfersDataDelete
          }
          linkToView="/dashboard/inventory/products-transfers/view/"
          linkToCreate="/dashboard/inventory/products-transfers/new"
          endPoint="products/transfer-products"
          titleToCreate="products transfers"
          resourceName="Product Transfer"
        />
        </div>
  )
}
