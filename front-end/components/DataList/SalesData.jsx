...
import { saleCashColumn, saleDelayColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"
import CheckBoxInput from "../FormInput/CheckBoxInput"

export default function SalesData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
  ...
  return (
     <div>
            <FixedHeader
              linkUrl="/dashboard/sales/new"
              textValue="Sales"
              numberData={newData?.length | 0}
              isHidden={!permission?.salesDataCreate }
            />
          ...
          <DataTable
            data={newData}
            columns={customerCash ? saleCashColumn : saleDelayColumn}
            onDeleted={permission?.salesDataDelete}
            onViewInvoice={permission?.salesDataViewInvoice}
            linkToCreate="/dashboard/sales/new"
            titleToCreate="sales"
            linkViewInvoice="/dashboard/sales/invoice/"
            endPoint="sales"
            resourceName="Sale"
          />
        </div>
