...
import {  transferMonyColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function TransfersMonyData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
 ...
 const newData = useMemo(()=>{
  ...
  return (
     <div>
    <FixedHeader
            linkUrl="/dashboard/safes/transfers/new"
            textValue="Mony Transfers"
            numberData={newData?.length | 0}
            isHidden={!permission?.safesTransfersCreate}
          />
             <div className=' overflow-auto md:mt-6 mt-4  md:mx-5 mx-3 '>
                   ...
                   <DataTable
                     data={newData}
                     columns={transferMonyColumn}
                     onView={permission?.safesTransfersView}
                     onDeleted={permission?.safesTransfersDelete}
                     linkToView="/dashboard/safes/transfers/view/"
                     linkToCreate="/dashboard/safes/transfers/new"
                     titleToCreate="mony transfers"
                     endPoint="transfers-mony"
                     resourceName="Transfer Mony"
                   />
          </div>
  )
}
