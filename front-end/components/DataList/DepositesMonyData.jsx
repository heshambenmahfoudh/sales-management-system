...
import { depositeMonyColumn } from "@/app/(dummy)/data"
import { useMemo } from "react"
import { spliteObject } from "@/lib/spliteItem"

export default function DepositesMonyData({data,permission}) {
  const {
    register,
    watch,
    formState: { errors },
  ...
  return (
     <div>
   <FixedHeader
        linkUrl="/dashboard/safes/deposits/new"
        textValue="Mony Deposits"
        numberData={newData?.length | 0}
        isHidden={!permission?.safesDepositsCreate}
      />
           ...
           <DataTable
            data={newData}
            columns={depositeMonyColumn}
            onView={permission?.safesDepositsView}
            onDeleted={permission?.safesDepositsDelete}
            linkToView="/dashboard/safes/deposits/view/"
            linkToCreate="/dashboard/safes/deposits/new"
            titleToCreate="mony deposits"
            endPoint="mony-deposites"
            resourceName="Mony Deposite"
        />
        </div>
  )
}
